...
public class SongFileAccessor extends FileAccessor{
  ...
  */
   public void processLine(String line){
      String[] songItem = line.split(",");
      String title = songItem[0];
      String album = songItem[1];
      String artist = songItem[2];
      double playTime = Double.parseDouble(songItem[3]);
      Song songObject;
      if (album.length()==0) {
         songObject = new Song(title,artist,playTime);
      } 
      else {
         songObject = new Song(title,album,artist,playTime);
      }
      songs.addSong(songObject);    
   }
  ...
  */   
   public String songToCSVString(Song song){
      String csv = "" ;
      csv += song.getTitle() + "," + song.getAlbum()+ "," + song.getArtist() + ","  + song.getPlayTime();
      return csv;
   }
  ...
  */
   public String getSongsAsCSV(SongList songList){
      String csvSong = "";
      ArrayList<Song> listOfSongs = songList.getSongList();
      for (int i=0; i<listOfSongs.size(); i++) {
         csvSong += songToCSVString(listOfSongs.get(i)) + "\n";
      }
      return csvSong;
   }
